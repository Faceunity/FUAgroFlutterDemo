...
- (void)consumerWithData:(id)model viewModelBlock:(ViewModelBlock _Nullable)ViewModelBlock {
    ...
    [self.selectedViewModel consumerWithData:model viewModelBlock:ViewModelBlock];
}
...
- (void)setDefaultParams {
    ...
    if ([(NSArray *)self.filtersViewModel.provider.dataSource count] > 1) {
        //默认选择第一个滤镜
        [self.filtersViewModel consumerWithData:[(NSArray *)self.filtersViewModel.provider.dataSource objectAtIndex:1] viewModelBlock:nil];
    }
